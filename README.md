...
- проверяет наличие столбца `id`, `ID` или `Номер`;
- если такого столбца нет, автоматически добавляет `id` и нумерует строки с 1;
...
- Если в таблице нет столбца `id`, `ID` или `Номер`, программа добавит первый столбец `id` и заполнит его числами `1, 2, 3...`.
